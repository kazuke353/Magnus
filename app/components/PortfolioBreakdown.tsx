import { useMemo } from 'react';
    ...
    interface Instrument {
      ...
      pieCurrentAllocation: number;
      pieTargetAllocation: number;
    }
    ...
    interface PortfolioBreakdownProps {
      ...
      currency: string;
      className?: string;
      onSelectItem: (ticker: string) => void; // Add the new prop
    }
            ...
            <Card key={index} title={portfolio.name} className="shadow-md rounded-xl bg-gray-50 dark:bg-gray-800 p-6">
              <div className="space-y-4">
                ...
                <SortableTable
                  ...
                  keyField="ticker"
                  onRowClick={(item) => onSelectItem(item.ticker)} // Pass the handler to SortableTable
                  columns={[
                    {
                      key: "fullName",
                      header: "Instrument",
                      ...
                      filterable: true,
                      render: (item) => (
                        // Make the name clickable as well
                        <span
                          className="font-medium text-gray-900 dark:text-gray-100 cursor-pointer hover:text-blue-600 dark:hover:text-blue-400"
                          onClick={(e) => { e.stopPropagation(); onSelectItem(item.ticker); }} // Prevent row click propagation if needed
                        >
                          {item.fullName}
                        </span>
                      )
                    ...
                    // ... other performance columns remain the same ...
                     {
